...
export default function OnboardingPage() {
  ...
  useEffect(() => { let active = true; void createSupabaseBrowserClient().from('businesses').select('id').limit(1).maybeSingle().then(({ data, error: queryError }) => { if (active && !queryError && data) router.replace('/dashboard'); }); return () => { active = false; }; }, [router]);
