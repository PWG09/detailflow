    ...
    <div className="shell quote-layout">
      ...
      <section className="quote-form-wrap">
        ...
        <form onSubmit={(event) => { event.preventDefault(); if (step < 3) setStep(step + 1); else void submitQuote(event.currentTarget); }}>
          {step === 1 && <><h2>Start with the basics.</h2><p className="form-intro">First, tell us what you are bringing in.</p><div className="form-grid"><div className="field"><label htmlFor="year">Year</label><input id="year" name="year" placeholder="2021" required /></div><div className="field"><label htmlFor="makeModel">Make and model</label><input id="makeModel" name="makeModel" placeholder="Porsche Macan" required /></div><div className="field full"><label>Vehicle type</label><div className="service-options">{vehicles.map((item) => <button type="button" className={'service-option ' + (vehicleType === item ? 'selected' : '')} onClick={() => setVehicleType(item)} key={item}><strong>{item}</strong></button>)}</div></div></div><div className="form-actions"><span className="mono" style={{ fontSize: 11, color: 'var(--muted)' }}>1 of 3</span><button className="button-primary" type="submit">Continue <ArrowRight size={15} /></button></div></>}
          {step === 2 && <><h2>Choose a service.</h2><p className="form-intro">Select what you are looking to have done.</p><div className="service-options">{services.map(([name, price]) => <button type="button" className={'service-option ' + (service === name ? 'selected' : '')} onClick={() => setService(name)} key={name}><strong>{name}</strong><small>{price}</small></button>)}</div><div className="field"><label htmlFor="condition">Anything we should know?</label><textarea id="condition" name="condition" rows={4} placeholder="Pet hair, stains, paint concerns, or anything else..." /></div><div className="form-actions"><button className="back-button" type="button" onClick={() => setStep(1)}><ArrowLeft size={14} /> Back</button><button className="button-primary" type="submit">Add your details <ArrowRight size={15} /></button></div></>}
          {step === 3 && <><h2>Where should we send your estimate?</h2><p className="form-intro">Your photos help us understand the job. They are only shared with Northline Detailing.</p><div className="field"><label className="service-option" style={{ display: 'flex', alignItems: 'center', gap: 12 }}><Upload size={20} color="var(--orange)" /><span><strong>Upload vehicle photos</strong><small>Up to 8 JPG or PNG images</small></span><input type="file" name="photos" multiple accept="image/png,image/jpeg" style={{ display: 'none' }} /></label></div><div className="form-grid"><div className="field"><label htmlFor="firstName">First name</label><input id="firstName" name="firstName" placeholder="Jordan" required /></div><div className="field"><label htmlFor="lastName">Last name</label><input id="lastName" name="lastName" placeholder="Lee" required /></div><div className="field"><label htmlFor="email">Email</label><input id="email" name="email" type="email" placeholder="jordan@email.com" required /></div><div className="field"><label htmlFor="phone">Phone</label><input id="phone" name="phone" placeholder="(555) 014-8820" required /></div></div><div className="estimate"><small>your current estimate</small><strong>{service === 'Ceramic coating' ? '$800+' : service === 'Full detail' ? '$250-350' : '$150-220'}</strong><p>Based on a {service.toLowerCase()} for a {vehicleType}. Final pricing is confirmed after review.</p></div><div className="form-actions"><button className="back-button" type="button" onClick={() => setStep(2)}><ArrowLeft size={14} /> Back</button><button className="button-primary" type="submit" disabled={submitting}>{submitting ? 'Submitting...' : 'Request my estimate'} <ArrowRight size={15} /></button></div></>}
        </form>
      </section>
    </div>
  </main>;
