...
export async function GET() {
  ...
  const { data, error } = await supabase.from('customers').select('id,first_name,last_name,email,phone,notes,created_at,updated_at').eq('business_id', membership.business_id).order('updated_at', { ascending: false }); if (error) return NextResponse.json({ error: 'Unable to load customers.' }, { status: 500 }); return NextResponse.json(data);
}
