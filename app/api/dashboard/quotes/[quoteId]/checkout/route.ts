...
export async function POST(_request:Request,{params}:{params:Promise<{quoteId:string}>}){
 ...
 try{
  const stripe=getStripe();const session=(await stripe.checkout.sessions.create({mode:'payment',customer_email:customer?.email||undefined,line_items:[{price_data:{currency,product_data:{name:`Quote ${quote.quote_number}`},unit_amount:amount},quantity:1}],metadata:{quoteId:quote.id,businessId:membership.business_id,connectedAccountId:business.stripe_connected_account_id,platformFeeAmount:String(fee)},payment_intent_data:{application_fee_amount:fee,transfer_data:{destination:business.stripe_connected_account_id},metadata:{quoteId:quote.id,businessId:membership.business_id}},success_url:`${getAppUrl()}/quote/payment/success?session_id={CHECKOUT_SESSION_ID}`,cancel_url:`${getAppUrl()}/quote/payment/cancelled`},{idempotencyKey:`quote-payment:${quote.id}`})) as unknown as Stripe.Checkout.Session;
