...
export async function POST(_request:Request,{params}:{params:Promise<{quoteId:string}>}){
 ...
 const {data:quote,error}=await supabase.from('quotes').select('id,quote_number,total,status,payment_status,stripe_checkout_session_id,customers(email,first_name,last_name),businesses(name,currency,stripe_connected_account_id,stripe_connect_status,stripe_charges_enabled,stripe_payouts_enabled,platform_fee_percent)').eq('id',parsed.data.quoteId).eq('business_id',membership.business_id).single();
 if(error||!quote)return NextResponse.json({error:'Quote not found.'},{status:404});if(quote.status!=='accepted')return NextResponse.json({error:'Only accepted quotes can be paid.'},{status:400});if(quote.payment_status==='paid')return NextResponse.json({error:'This quote is already paid.'},{status:409});
 if(quote.stripe_checkout_session_id){try{const s=await getStripe().checkout.sessions.retrieve(quote.stripe_checkout_session_id);if(s.url)return NextResponse.json({url:s.url});}catch{}}
 ...
 const business=Array.isArray(quote.businesses)?quote.businesses[0]:quote.businesses;const customer=Array.isArray(quote.customers)?quote.customers[0]:quote.customers;const currency=(business?.currency||'USD').toLowerCase();
 ...
 try{
  const stripe=getStripe();const session=await stripe.checkout.sessions.create({mode:'payment',customer_email:customer?.email||undefined,line_items:[{price_data:{currency,product_data:{name:`Quote ${quote.quote_number}`},unit_amount:amount},quantity:1}],metadata:{quoteId:quote.id,businessId:membership.business_id,connectedAccountId:business.stripe_connected_account_id,platformFeeAmount:String(fee)},payment_intent_data:{application_fee_amount:fee,transfer_data:{destination:business.stripe_connected_account_id},metadata:{quoteId:quote.id,businessId:membership.business_id}},success_url:`${getAppUrl()}/quote/payment/success?session_id={CHECKOUT_SESSION_ID}`,cancel_url:`${getAppUrl()}/quote/payment/cancelled`},{idempotencyKey:`quote-payment:${quote.id}`});
