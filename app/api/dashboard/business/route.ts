...
export async function GET() { const { supabase, businessId } = await context(); if (!businessId) return NextResponse.json({ error: 'Workspace not found.' }, { status: 404 }); const { data, error } = await supabase.from('businesses').select('id,name,slug,email,phone,description,currency,tax_rate').eq('id', businessId).single(); if (error) return NextResponse.json({ error: 'Unable to load business profile.' }, { status: 500 }); return NextResponse.json(data); }

export async function PATCH(request: Request) { const { supabase, businessId } = await context(); if (!businessId) return NextResponse.json({ error: 'Workspace not found.' }, { status: 404 }); const parsed = businessSchema.safeParse(await request.json().catch(() => null)); if (!parsed.success) return NextResponse.json({ error: parsed.error.issues[0]?.message ?? 'Invalid business profile.' }, { status: 400 }); const { data, error } = await supabase.from('businesses').update({ ...parsed.data, updated_at: new Date().toISOString() }).eq('id', businessId).select('id,name,slug,email,phone,description,currency,tax_rate').single(); if (error) return NextResponse.json({ error: error.code === '23505' ? 'That public link is already taken.' : 'Unable to save business profile.' }, { status: 500 }); return NextResponse.json(data); }
